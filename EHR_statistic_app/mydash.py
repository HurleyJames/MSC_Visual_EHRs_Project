...
def make_data(request):
    ...
    """
    # -----------------------------------------------------------------
    dataset = pd.read_csv(os.getcwd() + r'/EHR_statistic_app/testdata.csv')
    print('i need', dataset.info(null_counts=True))
    year_num = request.POST.get('year_num')
    len_data, dataset_make = data_analyse(dataset)
    dataset_make_T = pd.DataFrame(dataset_make.values.T, index=dataset_make.columns, columns=dataset_make.index)
    dataset_index = dataset_make.columns.values
    year_list = dataset[year_num].unique()
    col_list = dataset_make.index.values
    constancy = distributions(dataset_make)
    # -----------------------------------------------------------------
    ...
    app.layout = html.Div([
        ...
        html.Div([
            html.Div([
                ...
                dcc.RadioItems(
                    id='sort_data',
                    options=[{'label': i, 'value': i} for i in dataset_index],
                    value='distinct',
                    labelStyle={'display': 'inline-block'}
                ),
                dcc.RadioItems(
            ...
            html.Div([
                dcc.Dropdown(
                    ...
                    options=[{'label': i, 'value': i} for i in
                             ['distinct', 'row_len', 'null_len', 'unique', 'value_length']],
                    value=''
                ),
            ], style={'width': '50%', 'float': 'center', 'display': 'inline-block'}),
            html.Div([
    ...
    def update_chart3(df):
        ...
        fig3.add_annotation(
            x=0.5, y=1, xanchor='left', yanchor='bottom', xref='paper', yref='paper', showarrow=False,
            text='null values'
        )
    ...
    def update_chart6(constancy_type):
        ...
        if constancy_type == 'Bar':
            fig_constancy = px.bar(y=constancy, x=dataset_make.columns.values, barmode="group")
        elif constancy_type == 'Line':
            fig_constancy = px.line(y=constancy, x=dataset_make.columns.values)
    ...
    def update_chart_select(sort_type, sort_data, year):
        len_data, dataset_make = data_analyse(dataset[dataset[year_num] == int(year)])
        if sort_type == 'asc':
            dataset_make_sort = dataset_make.sort_values(by=sort_data, ascending=True, inplace=False)
        elif sort_type == 'desc':
            dataset_make_sort = dataset_make.sort_values(by=sort_data, ascending=False, inplace=False)
        return update_chart1(dataset_make_sort)
    ...
    def update_chart_select(sort_type, sort_data, year):
        ...
        if sort_type == 'asc':
            ...
            dataset_make_sort = dataset_make.sort_values(by=sort_data, ascending=False, inplace=False)
        return update_chart2(dataset_make_sort)
    ...
    def update_chart_select(sort_type, sort_data, year):
        ...
        if sort_type == 'asc':
            ...
            dataset_make_sort = dataset_make.sort_values(by=sort_data, ascending=False, inplace=False)
        return update_chart3(dataset_make_sort)
    ...
    def update_chart_select(sort_type, sort_data, year):
        ...
        if sort_type == 'asc':
            ...
            dataset_make_sort = dataset_make.sort_values(by=sort_data, ascending=False, inplace=False)
        return update_chart4(dataset_make_sort)
    ...
    def update_chart_select(chart, chart_T_type, chart_T_sort):
        if chart_T_sort == 'asc':
            dataset_make_T_sort = dataset_make_T.sort_values(by=chart, ascending=True, inplace=False)
        elif chart_T_sort == 'desc':
            dataset_make_T_sort = dataset_make_T.sort_values(by=chart, ascending=False, inplace=False)
        return update_chart_T(dataset_make_T_sort, chart, chart_T_type)
    ...
    def update_chart_select(chart, chart_mini_type, chart_mini_sort):
        ...
        if chart_mini_type != 'Box' and chart != 'value_length':
            if chart_mini_sort == 'asc':
                dataset_make_sort = dataset_make.sort_values(by=chart, ascending=True, inplace=False)
            elif chart_mini_sort == 'desc':
                dataset_make_sort = dataset_make.sort_values(by=chart, ascending=False, inplace=False)
            len_data = ''
        else:
            len_data, dataset_make_sort = data_analyse(dataset)
        return update_chart(dataset_make_sort, chart, chart_mini_type, len_data)
...
def datagrid(request):
    try:
        dataset = pd.read_csv(os.getcwd() + r'/EHR_statistic_app/testdata.csv', error_bad_lines=False,
                              lineterminator="\n")
    except:
        dataset = pd.read_csv(os.getcwd() + r'/EHR_statistic_app/testdata.csv', error_bad_lines=False,
                              lineterminator="\n")
        ...
        dataset = pd.read_csv(os.getcwd() + r'/EHR_statistic_app/testdata.csv', error_bad_lines=False,
                              lineterminator="\n")
    list = dataset.to_dict(orient='records')
    year_list = dataset.columns.values
...
def data_analyse(dataset):
    ...
    dataset_make = pd.DataFrame(index=dataset.columns.values)
    distinct_res = []
    row_len_res = []
    null_len_res = []
    unique_res = []
    for i in dataset.columns.values:
        distinct_res.append(len(dataset[i].value_counts()))
        row_len_res.append(len(dataset[i]))
        unique_res.append(np.sum((dataset[i].value_counts().values == 1) != 0))
    dataset_make['distinct'] = distinct_res
    dataset_make['row_len'] = row_len_res
    dataset_make['null_len'] = dataset.isnull().sum(axis=0).values
    dataset_make['unique'] = unique_res
    return len_data, dataset_make


def distributions(dataset_make):
    """
    分布
    """
    constancy = []
    length = len(dataset_make)
    for i in dataset_make.columns.values:
        res_list = dataset_make[i].value_counts().sort_values(ascending=False, inplace=False).values.tolist()
        if res_list:
